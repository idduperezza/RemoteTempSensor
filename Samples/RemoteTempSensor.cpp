...
#include "mbed.h"
#include "DHT.h"
DHT sensor(PA_10, DHT::DHT11);
int erroreTest = 0;
DigitalOut alimentazione(P_PA_2_TX);
...
RemoteTempSensor::SensorUpdate(bool keyPressed)
{
	...
	if (sensorSI7021->hasSensor()) {
        ...
        */
        ...
        // test
        alimentazione.write(1);
        thread_sleep_for(2000);
        erroreTest = sensor.read();
        if(erroreTest == DHT::SUCCESS){
            sensorData.humidity = sensor.getHumidity();
            sensorData.temperature = sensor.getTemperature();
            dprintf("UMIDITA' RILEVATA: %d, TEMPERATURA RILEVATA: %d", sensorData.humidity, sensorData.temperature);
        } else {
            dprintf("ERRORE NEL SENSORE: %d", erroreTest); 
        }
        alimentazione.write(0);
	}
#endif
...
RemoteTempSensor::MessageRecvHandler(int AppID, RadioShuttle::devid_t stationID, int msgID, int status, void *buffer, int length)
{
    ...
    if (gotData) {
    	...
    	if (length == sizeof(sensorDataRecord) && r->magic == TEMP_SENSOR_DATA_MAGIC) {
    		...
    		dprintf("IL MESSAGGIO CONTENEVA DEI DATI: ID: %d Temperature: %.2f%sC  Humidity: %d%%  Battery: %f V", (int)stationID, temp, &cel[0], r->humidity, volt);
        }
    }
}
